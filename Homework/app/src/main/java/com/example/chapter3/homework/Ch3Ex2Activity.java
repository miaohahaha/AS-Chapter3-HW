...
public class Ch3Ex2Activity extends AppCompatActivity {
    ...
    private void resetTargetAnimation() {
        ...
        ObjectAnimator alphaXAnimation = ObjectAnimator.ofFloat(target,"alpha",1.0f,0.5f);
        alphaXAnimation.setRepeatCount(ValueAnimator.INFINITE);
        alphaXAnimation.setDuration(Integer.parseInt(durationSelector.getText().toString()));
        alphaXAnimation.setRepeatMode(ValueAnimator.REVERSE);
        alphaXAnimation.setInterpolator(new LinearInterpolator());

        // TODO ex2-3: 将上面创建的其他 ObjectAnimator 都添加到 AnimatorSet 中
        animatorSet = new AnimatorSet();
        animatorSet.playTogether(animator1,scaleXAnimator,scaleYAnimator,alphaXAnimation);
        animatorSet.start();
    }
}
